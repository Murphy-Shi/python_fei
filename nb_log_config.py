...
LOG_PATH = Path(__file__).absolute().parent.parent / Path("configure",
                                                          "pythonlogs")  # 这么配置就会自动在你项目的根目录下创建pythonlogs文件夹了并写入。
...
FORMATTER_DICT = {
    ...
    5: logging.Formatter('%(asctime)s "%(pathname)s:%(lineno)d" %(levelname)s %(message)s',
    ...
    8: JsonFormatterJumpAble('%(asctime)s - %(name)s - %(levelname)s - %(message)s - "%(filename)s %(lineno)d -" ',
                             "%Y-%m-%d %H:%M:%S", json_ensure_ascii=False),  # 这个是json日志，方便分析.
